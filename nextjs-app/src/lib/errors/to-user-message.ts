...
export async function withErrorHandling(
  ...
  try {
    ...
    log({ requestId, ...context }, response.status, Date.now() - startedAt, 'success');
    return response;
  } catch (err) {
    ...
    if (appErr.code === 'INTERNAL' && !(err instanceof AppError)) {
      console.error(JSON.stringify({ requestId, route: context.route, unexpected: String(err) }));
    }
    log({ requestId, ...context }, appErr.httpStatus, Date.now() - startedAt, 'error', appErr.code);
