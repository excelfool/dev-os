...
Body `{}`. GPT-4o, JSON mode, temp 0.1, `max_tokens` 2000, `user=<user_id>`, 20 s per-attempt timeout, up to 3 attempts + 1 JSON-repair retry, all inside a **24 s in-handler deadline** (spec 06 §3a) so the 26 s Netlify ceiling can never kill the run. Terms are persisted through the `persist_key_terms` RPC in a single transaction. A run stuck in `processing` for over 5 minutes is re-claimed by the next call and force-failed by the `reclaim-stale-processing` job. **v1.1 D49 a (spec 06 v1.1 §G):** when `PROCESS_JOB_SECRET` is set the route keeps every guard and the claim, hands the run to the `process-background` Netlify function (HMAC-signed) and returns **`202 { contract_id, status: 'processing' }`**; the results page polls `GET /api/contracts/{id}` until `status` leaves `processing`. Without the secret (local dev, tests) it runs inline and returns `200` as below.
