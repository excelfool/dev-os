...
**Note (2026-09-23) — the injection patterns as built.** The `inj.*` ids above are implemented in `src/lib/security/patterns/injection.ts` with the Lab 3 regexes (formerly `detectPromptInjection`, security plan §1 finding 1), which keep their **anchoring**: each requires an imperative aimed at the assistant at the start of a message or clause, so "Can the supplier act as an agent?" and "Does clause 7 override the previous agreement instructions?" are not blocked. The quoted phrases in the table are the vocabulary, not the regexes. One pattern is deliberately **wider** than Lab 3: `inj.system_prompt` matches a bare mention of "system prompt", "system message" or "developer message", as named here, since contract questions do not use those words. Lab 3's jailbreak phrases ("developer mode", "DAN mode") are filed under `inj.role_override`.
