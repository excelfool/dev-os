...
| **Chat answer sometimes renders twice** | Mechanism **confirmed by instrumentation**, not reproduced deterministically. Render-only: measured at 2 runs in 5, with exactly one POST, one model call and exactly two rows persisted. | `useChat` starts a history GET on mount and applies its result unconditionally — it is not discarded once a turn has begun. Logging the hook's state transitions caught the duplicating sequence directly: `load resolved cancelled=false serverRows=2` → `load REPLACE prev=1 -> 2` (the server list already contains the answer) → `send APPEND prev=2 -> 3` with the SAME assistant id twice. The same cause has a second symptom: when the load resolves with zero rows mid-turn, `load REPLACE prev=1 -> 0` wipes the user's own question. **Three attempts to force that interleaving from Playwright failed** — StrictMode issues two GETs per mount and the live effect is not reliably the second to resolve, and holding both does not order the load's state update ahead of the turn's append. The level where this is deterministic is a hook test, which needs `jsdom` + `@testing-library/react`; neither is installed, so adding them is a dependency decision. Reported rather than fixed: no regression test can be claimed, and an unverifiable fix is worse than a known open item. |
