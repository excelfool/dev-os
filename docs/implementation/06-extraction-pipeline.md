...
| Input (bounded to hold the ≤ $0.03 budget at GPT-4o pricing) | (1) the persisted terms as compact JSON `{ term_name, value, page_number, source_sentence }` (≈ 1,500 tokens for 36 terms); (2) a **3,000-token window** of `contract_text`: the first page(s) up to 3,000 tokens (`[PAGE N]` markers kept) — recitals, parties and the core obligations live there; the full text is **not** sent. ≈ 4,800 input + ≤ 400 output tokens ≈ **$0.030** |
