...
**Note (2026-09-23, G43).** When a caller's `deadlineAt` leaves less than the 6 s minimum before the **first** attempt, `callLlm` now throws **`AI_TIMEOUT`** (504), not `AI_UNAVAILABLE` (503): the caller's budget ran out, the provider was never tried. A deadline reached after an attempt failed with a non-timeout error still reports that error (`AI_UNAVAILABLE`). Applies to every purpose; found while giving the chat turn its 15 s budget (spec 08 §3).
...
**v1.1 L5 (Stage 5d-1, 2026-09-22) — retryable timeouts and a 45 s extraction call.** Two defects, found on live contract `93ba9b49` (batch B timed out at 20 s on attempt 1, the job failed `AI_TIMEOUT` after 20.6 s of its 120 s budget, and nothing was retried):

1. **Timeouts and connection errors were never retried, on any purpose.** The OpenAI SDK models `APIUserAbortError` (which is how our own per-attempt abort surfaces), `APIConnectionError` and `APIConnectionTimeoutError` as `APIError` subclasses whose `status` is `undefined`. `isRetryable` tested `instanceof APIError` first and returned `status === 429 || status >= 500`, so all three fell out as non-retryable and the "timeouts and network errors" fall-through was unreachable. The classifier now tests `isTimeout` and `APIConnectionError` **before** the status check: timeouts and connection failures retry, `429` and `≥ 500` retry, every other status does not.
2. **Extraction ran on the 20 s general timeout**, which was sized for the inline 24 s deadline of §3a, while a real 36-term MSA batch runs 14–16 s. Extraction and its JSON-repair call now use **`OPENAI_EXTRACTION_TIMEOUT_MS` (default 45 s)**; `OPENAI_TIMEOUT_MS` stays **20 s** for chat and the query enhancer, and the summary keeps its own budget (§B, L4). The longer timeout cannot lengthen an inline request: `callLlm` still caps every attempt at `min(timeout, remaining − 1 s)` and starts no attempt with under 6 s left, so under the 24 s deadline attempt 1 is **23 s** and there is no second attempt, while under the background job's 120 s budget the batch gets the full 45 s.

**One retry layer.** The SDK client was constructed with its default `maxRetries: 2`, so a failed call was retried inside the SDK — invisibly, writing no `openai_calls` row and multiplying the per-attempt timeout underneath our deadline arithmetic. The client is now built with **`maxRetries: 0`**; `callLlm`'s loop is the only retry layer, so every attempt is recorded and budgeted. Step 7 of §3 reads: JSON mode, temperature 0.1, `max_tokens` 4000 per batch, **45 s** per-attempt timeout, up to 3 attempts within the caller's deadline.

**v1.1 L6 (Stage 5d-1b, 2026-09-22) — rate limits are honoured, not guessed, and every failed attempt is named.**

**D51 — Tier 2 is the operating assumption.** The organization moved to OpenAI Usage Tier 2 on 2026-09-22: gpt-4o is now **450,000 TPM / 5,000 RPM** (was 30,000 / 500). One 36-term MSA run sends ~34k prompt tokens across its batches and summary (live `93ba9b49`: 11,774 + 11,839 + 10,655 in ~40 s), so a single user's run no longer approaches the ceiling. 429s stay reachable from concurrency and from the **eval runner** (Stage 7, ~380k tokens in a minute), so they are handled properly rather than assumed away.

**Diagnosis first.** Every failed attempt now writes one structured line before its `openai_calls` row: `{ llm: 'attempt_failed', purpose, attempt, name, status, code, retryAfterMs, latencyMs }`. `name` is the error's **class** (`RateLimitError`, `APIConnectionTimeoutError`, …) — the SDK sets no `.name`, so every error otherwise reads as plain `Error`, which is why `93ba9b49`'s summary repair call failing in **379 ms** could not be classified from the telemetry alone. The line carries identifiers and classifications only: no key material, no prompt text, no response body.

**429 backoff from the provider's clock.** The wait is read from the response headers in order: `retry-after-ms`, then `retry-after` (seconds), then the **larger** of `x-ratelimit-reset-tokens` / `x-ratelimit-reset-requests` (OpenAI duration strings — `"6m0s"`, `"1.2s"`, `"20ms"` — parsed by the exported pure helper `parseRateLimitDuration`, which returns `null` for anything it cannot read rather than acting on a misparse). The wait gets **±10% jitter** so a batch's parallel calls do not return in lockstep, and is **capped at 30 s**. With no usable header the existing 1/2/4 s schedule stands. A retry is only taken when `now + wait + 6 s` still fits before `deadlineAt`; otherwise the loop stops and throws exactly as before, so a long reset can never eat a budget it cannot then use. Worst case inside the 120 s job budget: two waits at the 30 s cap, **60 s** of waiting across three attempts.

**`insufficient_quota` is not a rate limit.** A 429 whose body `code` is `insufficient_quota` is a billing state, not a pacing one: it is **not retryable**, maps to `AI_UNAVAILABLE`, and is logged separately as `{ llm: 'insufficient_quota' }` because no amount of waiting clears it.

**`openai_calls.outcome` is unchanged.** Its CHECK allows only `('success','timeout','error','invalid_json')`, so a 429 is recorded as `'error'`; the schema is deliberately **not** migrated for this, and the `attempt_failed` line carries the rate-limit distinction instead.
...
| Settings | temperature **0.2**, `max_tokens` **500**, JSON mode **off** (Markdown out), single attempt, per-call timeout **20 s** (`OPENAI_SUMMARY_TIMEOUT_MS`), capped at the caller's remaining budget — L4 (040016f) raised it from 10 s |
...
**v1.1 L5 (Stage 5d-1, 2026-09-22) — the job now actually uses its budget.** Before L5 the background job could not outlive a single slow batch: the extraction call was capped at 20 s and a timeout was classified as non-retryable, so `93ba9b49` died `AI_TIMEOUT` at 20.6 s of a 120 s budget with one `openai_calls` row and no retry. With the §3 L5 fix the job spends the budget it was given — attempt 1 at 45 s, then retries with exponential backoff while ≥ 6 s remain, each attempt capped at `remaining − 1 s`. Worst case for one batch is therefore the budget itself (~119 s, one second short of `PROCESS_JOB_BUDGET_MS`), after which the pipeline persists `status='error'`/`AI_TIMEOUT` as before; the 5-minute `reclaim-stale-processing` rule of §G.4 is still comfortably longer. The two batches run in parallel, so the pair's wall time is the slower batch, not the sum.
