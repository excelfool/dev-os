...
**v1.1 5d-2b (2026-09-22) — results-page fixes from the live walk on contract `93ba9b49` (Expel.pdf, 8 pp).**

**L7 — the "Page X of N" indicator is measured, not inferred.** After `goToPage(5)` the viewer showed clause 10 (text layer: page 5) under "Page 3 of 8"; the same view had read "Page 6 of 8" minutes earlier. Navigation itself was always correct. The cause is that the indicator was set from the **lazy-render** `IntersectionObserver`, which runs with `rootMargin: '200% 0px'` so that canvases are drawn before they scroll into view — under that margin roughly five screens of pages are "intersecting", and the indicator followed whichever entry fired last. The indicator now reports the page occupying the **largest visible area** of the scroll container (ties → the lower page), recomputed on scroll and on resize, and set directly to the target on programmatic navigation so it neither waits for a smooth scroll nor flickers through the pages passed on the way. The observer keeps its margin and now drives rendering only. `TextViewer` gains the same indicator, computed by the same `useVisiblePage` hook, so the two viewers always agree.

**L8 — multi-page summary citations.** "[Page 1, 5, 7]" rendered as raw text while "[Page 2]" became a chip. The single-page pattern `\[Page\s+(\d+)\]` matched no multi-page form. Both the card and `summary-validation` now share one parser covering `[Page 3]`, `[Page 1, 5, 7]`, `[Pages 3–4]` (hyphen, en dash or em dash), `[Page 3, Page 5]` and `[Pages 1 and 2]`, and the card renders **one chip per page**, each calling `goToPage`. The validator was wrong in the same way and more consequentially: `citedPages` returned nothing for a multi-page citation, so a correctly cited sentence counted as **uncited** — triggering a repair call that was not needed and risking `summary_uncited=true` on a sound summary. The sentence splitter keeps a multi-page citation attached to its sentence. **No stored summary is regenerated**; existing rows simply render and validate correctly from now on.

**L11 — "Found on page N" names the page the MODEL cited.** The Source block took its page from the live `page_number`, so correcting a term's page silently rewrote where the quote came from. It now reads `original_ai_page` (falling back to the current page when the model recorded none). The `PageChip` continues to show the user's edited page: the chip says where the term belongs, the Source block says where this sentence was found, and after an edit those are legitimately different.

See spec 21 §7.2 for **L10** (renewal roll-forward, D52) and `formatDateOnly` in `src/lib/utils/format.ts` for **L9**: `key_dates.date` is a calendar date, and rendering it through `new Date('2022-09-21')` in a negative-offset zone moved every date back a day — the card showed 20 Sep 2022 for a stored 2022-09-21 to a viewer in UTC−4. Every date-only value now goes through one helper that builds the date from its parts, so no offset is ever applied.
